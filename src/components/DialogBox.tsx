...
const DialogBoxScheme = z.object({
  language: z.string(),
  checkedLanguages: z.array(z.string()).min(1, 'Выберите хотя бы один язык.'),
});

export const DialogBox = ({ lang }: DialogBoxProps) => {
  ...
  const zodForm = useForm<z.infer<typeof DialogBoxScheme>>({
    resolver: zodResolver(DialogBoxScheme),
    defaultValues: {
      language: lang,
  ...
  const onSubmit: SubmitHandler<z.infer<typeof DialogBoxScheme>> = (
    formData,
  ) => {
