...
export const ContentImageProvider: FC<{ children: ReactNode }> = ({children}) => {
    const replaceImagesInContent = (content: string) => {
        if (!content) return null;

        return parse(content, {
            replace: (domNode: any) => {
                if (domNode.name === 'img' && domNode.attribs?.id?.startsWith('content-image')) {
                    return (
                        <DefaultImage
...
export const useContentImageContext = () => {
    ...
    if (!context) {
        throw new Error('useContentImageContext must be used within a ContentImageProvider');
    }
    return context;
};